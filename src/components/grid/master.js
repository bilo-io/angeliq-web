...
export class MasterGrid extends Component {
    static defaultProps = {
        items: [],
        isDroppable: true,
        onDropItem: (i) => console.log('UNDEFINED: <MasterGrid/>.onDropItem = (elemParams) => {}'),
    ...
    render () {
        ...
        const TileControls = ({ index }) => <div className='flex-row space-between' style={{
            ...
            <div className='flex-row'>
                <img src={ getAppIcon(items[index].type) } style={{ width: '1.5rem', height: '1.5rem', margin: '0.25rem' }}/>
                <div style={{ display: 'block' }}>{ getApp(items[index].type).name }</div>
            </div>
            <div className='flex-row'>
                                ...
                                tile.type.includes('widget')
                                    ...
                                    : tile.type.includes('chart')
                                        ...
                                        : <Empty
                                            ...
                                            onClick={ this.editTile(i) }
                                            style={ isActive ? { background: (getApp(items[i].type) || {}).meta.banner, height: 'calc(100% - 2rem)' } : undefined }
                                        />
                            }
