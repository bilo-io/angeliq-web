...
export class Explorer extends Component {
    ...
    toggleToolbar = () => this.setState({ isToolbarOpen: !this.state.isToolbarOpen })

    toggleCollection = i => () => this.setState({
        resourcesByCollection: {
    ...
    deselect = () => this.setState({ activeResource: undefined, activeResourceIndex: -1 })
    ...
    setResourcesByCollection = (resources) => {
        ...
        collections.forEach((collection, i) => {
            const previousStateFolder = this.state.resourcesByCollection[collection.id]
            resourcesByCollection[collection.id] = {
                resources: resources.filter((presentation, i) => presentation.collectionId === collection.id),
                isOpen: (previousStateFolder && previousStateFolder.isOpen) || false
            }
        })

        const resourcesWithoutCollection = resources.filter((presentation) => !presentation.collectionId)
    ...
    filterResource = (query) => {
        ...
        const filteredSources = resources.filter(resource => {
            const isMatchingName = resource.name.toLowerCase().includes(query.toLowerCase())
    ...
    render () {
        ...
        return <div className='vision-explorer'>
            <div className='padded flex-row auto-scroll-x' style={paddedSlot}>
                <div style={{ flexGrow: 1 }}>
                    <SearchResource
                        onChange={ this.filterResource }
                    />
                </div>
                <button
            ...
            {
                isToolbarOpen && <div>
                    <div className='flex-row space-between auto-scroll-x' style={paddedSlot}>
            ...
            }
            {
                isLoading
                    ...
                    : <div>
                        <div className='auto-scroll-y' style={{ ...paddedSlot, height: 'calc(100vh - 7rem)' }}>
                            {
                                Object.keys(resourcesByCollection).map((collection, i) => {
                                    const isCollectionEmpty = resourcesByCollection[collection].resources.length == 0
                                    const showEmptyCollections = false
                                    return (showEmptyCollections || !isCollectionEmpty) && <div key={ i }>
                                        <div key={ i } className='sidebar-collection'onClick={ this.toggleCollection(collection) }>
                                            <FAIcon name={resourcesByCollection[collection].isOpen ? 'folder-open' : 'folder' } />&nbsp;&nbsp;
                                            <div>
                                                <strong>
                                                    <Highlight
                                                        searchWords={[query]}
                                                        highlightClassName='react-highlight-words'
                                                        textToHighlight={ collections.filter(c => c.id === collection).pop().name }
                                                    />
                                                </strong>
                                            </div>
                                        </div>
                                        {
                                            resourcesByCollection[collection].isOpen && (resourcesByCollection[collection].resources || []).map((resource, s) => (
                                                <div key={ s }
                                                    className={`sidebar-item ${activeResource && resource.id === activeResource.id ? 'selected' : ''}`}
