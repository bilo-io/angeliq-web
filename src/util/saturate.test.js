...
describe('Saturate', () => {
    test('should saturate withDistribution', () => {
        ...
        expect(saturate.withDistribution({ items: users }, ['age'])).toMatchObject([[30, 31, 35]])
    })

    test('should saturate withStats', () => {
        expect(saturate.withStats({
            items: users
        }, ['email'], ['age', 'name'])).toMatchObject(formattedUsers)
    })

    test('should saturate withDimensions', () => {
        expect(saturate.withDimensions(users, ['email'], ['age', 'name'])).toMatchObject(
            {
                0: {
                    age: 30,
                    email: 'bilo.lwabona@gmail.com',
    ...
    test('should saturate withAll', () => {
        expect(saturate.withAll(
            users
            , ['email'], ['age', 'name'])).toMatchObject({
            stats: {
                age: {
                    avg: 32,
                    max: 35,
    ...
    test('should not saturate undefined', () => {
        expect(saturate.withAll(undefined, [], []))
            .toMatchObject({
                data: undefined,
